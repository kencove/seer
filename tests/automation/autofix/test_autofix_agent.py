...
def test_should_continue_waiting_for_user_response_with_fallback_config(
    autofix_agent: AutofixAgent, fallback_run_config: RunConfig
):
    """Test should_continue with fallback models configuration"""
    with autofix_agent.context.state.update() as state:
        state.steps = [
            DefaultStep(status=AutofixStatus.WAITING_FOR_USER_RESPONSE, key="test", title="Test")
        ]
    assert not autofix_agent.should_continue(fallback_run_config)
...
def test_should_continue_normal_case_with_fallback_config(
    autofix_agent: AutofixAgent, fallback_run_config: RunConfig
):
    """Test should_continue with fallback models configuration"""
    with autofix_agent.context.state.update() as state:
        state.steps = [DefaultStep(status=AutofixStatus.PROCESSING, key="test", title="Test")]
    assert autofix_agent.should_continue(fallback_run_config)
...
def test_should_continue_returns_false_when_max_iterations_reached_with_fallback(
    autofix_agent, fallback_run_config
):
    """Test max iterations with fallback models configuration"""
    autofix_agent.iterations = fallback_run_config.max_iterations
    autofix_agent.memory = [Message(role="user", content="You're great when you work")]
    with autofix_agent.context.state.update() as state:
        state.steps = [DefaultStep(status=AutofixStatus.PROCESSING, key="test", title="Test")]
    assert not autofix_agent.should_continue(fallback_run_config)
...
def test_should_continue_returns_false_with_stop_message_fallback(
    autofix_agent, fallback_run_config
):
    """Test stop message with fallback models configuration"""
    fallback_run_config.stop_message = "STOP"
    autofix_agent.memory = [Message(role="assistant", content="Let's STOP here")]
    autofix_agent.iterations = 1
    with autofix_agent.context.state.update() as state:
        state.steps = [DefaultStep(status=AutofixStatus.PROCESSING, key="test", title="Test")]
...
def test_should_continue_returns_false_when_assistant_message_has_no_tool_calls_fallback(
    autofix_agent, fallback_run_config
):
    """Test no tool calls condition with fallback models configuration"""
    autofix_agent.memory = [Message(role="assistant", content="All done!")]
    autofix_agent.iterations = 1
    with autofix_agent.context.state.update() as state:
        state.steps = [DefaultStep(status=AutofixStatus.PROCESSING, key="test", title="Test")]
...
@pytest.mark.vcr()
def test_run_iteration_with_queued_user_messages_fallback_config(
    interactive_autofix_agent,
    fallback_run_config,
):
    """Test run iteration with queued user messages using fallback models"""
    with interactive_autofix_agent.context.state.update() as state:
        state.steps.append(
            DefaultStep(
                status=AutofixStatus.PROCESSING,
    ...
    with interactive_autofix_agent.manage_run():
        interactive_autofix_agent.run_iteration(fallback_run_config)
...
@patch("seer.automation.autofix.autofix_agent.AutofixAgent.get_completion")
@pytest.mark.vcr()
def test_run_iteration_no_help_prompt_when_not_needed_fallback_config(
    mock_get_completion, interactive_autofix_agent, fallback_run_config
):
    """Test run iteration with no help prompt using fallback models"""
    mock_completion = MagicMock(
        message=Message(role="assistant", content="Thinking about the solution...")
    )
    mock_get_completion.return_value = mock_completion
...
@pytest.mark.vcr()
def test_get_completion_interrupts_with_queued_messages_fallback_config(
    interactive_autofix_agent, fallback_run_config
):
    """Test get_completion interruption with fallback models"""
    # Set up queued user messages
    with interactive_autofix_agent.context.state.update() as state:
        state.steps.append(
            DefaultStep(
    ...
    # Call get_completion and verify it returns None due to interruption
    result = interactive_autofix_agent.get_completion(fallback_run_config)
    assert result is None
...
def test_run_iteration_skips_duplicate_tool_calls_fallback_config(
    autofix_agent, fallback_run_config
):
    """
    Test duplicate tool call skipping with fallback models configuration.
    """
    autofix_agent.tools = [
        FunctionTool(
            name="fix_bug",
    ...
    completion = LlmGenerateTextResponse(
        message=completion_message,
        metadata=LlmResponseMetadata(
            model=fallback_run_config.models[0].model_name,
            provider_name=fallback_run_config.models[0].provider_name,
            usage=Usage(completion_tokens=1, prompt_tokens=1, total_tokens=2),
        ),
    )
    autofix_agent.get_completion = lambda rc: completion
